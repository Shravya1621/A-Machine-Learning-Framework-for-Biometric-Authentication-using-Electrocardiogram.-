...
def runSVM():
    ...
    text.insert(END,"SVM CLASSIFICATION REPORT: "+str(classification_report(y_test, predict))+"\n")
    text.insert(END,"SVM CONFUSION MATRIX : "+str(confusion_matrix(y_test, predict))+"\n\n")
...
def runCNN():
    ...
    classifier.add(Dense(output_dim = YY.shape[0], activation = 'softmax'))
